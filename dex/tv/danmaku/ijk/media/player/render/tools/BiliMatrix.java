...
public class BiliMatrix {
    ...
    public static boolean invertM(float[] mInv, int mInvOffset, float[] m, int mOffset) {
        ...
        if (det == DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN) {
            return false;
        }
    ...
    public static void frustumM(float[] m, int offset, float left, float right, float bottom, float top, float near, float far) {
        ...
        if (near <= DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN) {
            throw new IllegalArgumentException("near <= 0.0f");
        }
        if (far <= DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN) {
    ...
    public static void setRotateM(float[] rm, int rmOffset, float a, float x, float y, float z) {
        ...
        if (1.0f == x && DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN == y && DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN == z) {
            rm[rmOffset + 5] = c;
            rm[rmOffset + 10] = c;
            rm[rmOffset + 6] = s;
            rm[rmOffset + 9] = -s;
            ...
            rm[rmOffset + 8] = 0.0f;
            rm[rmOffset + 0] = 1.0f;
        } else if (DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN == x && 1.0f == y && DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN == z) {
            rm[rmOffset + 0] = c;
            rm[rmOffset + 10] = c;
            rm[rmOffset + 8] = s;
            rm[rmOffset + 2] = -s;
            ...
            rm[rmOffset + 9] = 0.0f;
            rm[rmOffset + 5] = 1.0f;
        } else if (DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN == x && DanmakuConfig.DANMAKU_SCREEN_DOMAIN_UNKNOWN == y && 1.0f == z) {
            rm[rmOffset + 0] = c;
            rm[rmOffset + 5] = c;
            rm[rmOffset + 1] = s;
            rm[rmOffset + 4] = -s;
