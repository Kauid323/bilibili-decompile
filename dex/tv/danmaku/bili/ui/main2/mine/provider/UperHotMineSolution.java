...
public class UperHotMineSolution implements IMinePageInfo {
    private String TAG = "UperHotMineSolution";
    private HotMenuItemSolution mMenuSolution = new HotMenuItemSolution();
    private IMineMenuItemRedirection mRedirection = new IMineMenuItemRedirection() { // from class: tv.danmaku.bili.ui.main2.mine.provider.UperHotMineSolution.1
        public void redirect(Activity context, MenuGroup.Item menuData) {
            UperHotMineSolution.this.linkTo(context, menuData);
        }
    ...
    private static class HotMenuItemSolution extends DefaultMineMenuSolution {
        HotMenuItemSolution() {
        }
        ...
        public void onMenuItemClick(Context context, MenuGroup.Item item) {
            Xpref.getDefaultSharedPreferences(context).edit().putBoolean("ui.main.NavigationFragment.is_show_hot_new" + BiliAccounts.get(context).mid(), false).apply();
            UperRepoter.ugcCenterHotClick(context);
        }
    }
}