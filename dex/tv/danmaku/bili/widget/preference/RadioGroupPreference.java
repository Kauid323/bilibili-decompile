...
public class RadioGroupPreference extends PreferenceCategory implements Preference.OnPreferenceClickListener {
    ...
    public void setRadioItems(CharSequence[] radioEntries, CharSequence[] radioEntryValues, String defaultRadioValue) {
        this.mRadioEntries = radioEntries;
        this.mRadioEntryValues = radioEntryValues;
        this.mDefaultRadioValue = defaultRadioValue;
        removeAll();
        initRadioButtons();
    }
    ...
    public RadioButtonPreference findPreferenceByRadioValue(String radioValue) {
        RadioButtonPreference radioButtonPref;
        String realValue;
        int count = getPreferenceCount();
        for (int i = 0; i < count; i++) {
            RadioButtonPreference preference = getPreference(i);
            if ((preference instanceof RadioButtonPreference) && (realValue = (radioButtonPref = preference).getRadioValue()) != null && realValue.equalsIgnoreCase(radioValue)) {
    ...
    /* JADX WARN: Type inference failed for: r1v1, types: [android.os.Parcelable, tv.danmaku.bili.widget.preference.RadioGroupPreference$SavedState] */
