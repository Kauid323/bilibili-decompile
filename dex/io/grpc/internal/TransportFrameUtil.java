...
public final class TransportFrameUtil {
    ...
    public static byte[][] toHttp2Headers(Metadata headers) {
        ...
        int i3 = serializedHeaders.length;
        if (k == i3) {
    ...
    public static byte[][] toRawSerializedHeaders(byte[][] http2Headers) {
        for (int i2 = 0; i2 < http2Headers.length; i2 += 2) {
            ...
            if (endsWith(key, binaryHeaderSuffixBytes)) {
                for (byte b : value) {
                    if (b == 44) {
                        return serializeHeadersWithCommasInBin(http2Headers, i2);
                    }
                }
    ...
    private static byte[][] serializeHeadersWithCommasInBin(byte[][] http2Headers, int resumeFrom) {
        ...
        for (int i2 = 0; i2 < resumeFrom; i2++) {
            headerList.add(http2Headers[i2]);
        }
        for (int i3 = resumeFrom; i3 < http2Headers.length; i3 += 2) {
            byte[] key = http2Headers[i3];
            byte[] value = http2Headers[i3 + 1];
            if (!endsWith(key, binaryHeaderSuffixBytes)) {
                headerList.add(key);
