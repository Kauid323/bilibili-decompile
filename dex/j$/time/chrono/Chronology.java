...
public interface Chronology extends Comparable<Chronology> {
    ...
    public final /* synthetic */ class CC {
        ...
        public static long $default$epochSecond(Chronology _this, int prolepticYear, int month, int dayOfMonth, int hour, int minute, int second, ZoneOffset zoneOffset) {
            Objects.requireNonNull(zoneOffset, "zoneOffset");
            ChronoField.HOUR_OF_DAY.checkValidValue(hour);
            ChronoField.MINUTE_OF_HOUR.checkValidValue(minute);
            ChronoField.SECOND_OF_MINUTE.checkValidValue(second);
            long daysInSec = Chronology$$ExternalSyntheticBackport0.m(_this.date(prolepticYear, month, dayOfMonth).toEpochDay(), 86400);
            long timeinSec = (((hour * 60) + minute) * 60) + second;
            return Clock$OffsetClock$$ExternalSyntheticBackport0.m(daysInSec, timeinSec - zoneOffset.getTotalSeconds());
        }

        public static long $default$epochSecond(Chronology _this, Era era, int yearOfEra, int month, int dayOfMonth, int hour, int minute, int second, ZoneOffset zoneOffset) {
            Objects.requireNonNull(era, "era");
            return _this.epochSecond(_this.prolepticYear(era, yearOfEra), month, dayOfMonth, hour, minute, second, zoneOffset);
        }
    }
}