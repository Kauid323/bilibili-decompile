            ...
            try {
                ...
                try {
                    if (job != null && !job.isActive()) {
                        ...
                        if (DebugKt.getRECOVER_STACK_TRACES()) {
                            try {
                                if (continuation instanceof CoroutineStackFrame) {
                                    cancellationException = StackTraceRecoveryKt.access$recoverFromStackFrame(cause, (CoroutineStackFrame) continuation);
                        ...
                        continuation.resumeWith(Result.constructor-impl(getSuccessfulResult$kotlinx_coroutines_core(state)));
                    }
                    ...
                    try {
                        ...
                        result = Result.constructor-impl(Unit.INSTANCE);
                    } catch (Throwable th3) {
                        th = th3;
                        Result.Companion companion5 = Result.Companion;
                        result = Result.constructor-impl(ResultKt.createFailure(th));
                        handleFatalException$kotlinx_coroutines_core(fatalException, Result.exceptionOrNull-impl(result));
                    }
                } catch (Throwable th4) {
                    th = th4;
                }
