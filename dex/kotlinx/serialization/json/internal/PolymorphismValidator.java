...
public final class PolymorphismValidator implements SerializersModuleCollector {
    ...
    private final void checkKind(SerialDescriptor descriptor, KClass<?> kClass) {
    ...
    private final void checkDiscriminatorCollisions(SerialDescriptor descriptor, KClass<?> kClass) {
        int elementsCount = descriptor.getElementsCount();
        for (int i = 0; i < elementsCount; i++) {
            String name = descriptor.getElementName(i);
            if (Intrinsics.areEqual(name, this.discriminator)) {
                throw new IllegalArgumentException("Polymorphic serializer for " + kClass + " has property '" + name + "' that conflicts with JSON class discriminator. You can either change class discriminator in JsonConfiguration, rename property with @SerialName annotation or fall back to array polymorphism");
            }
        }
    }
