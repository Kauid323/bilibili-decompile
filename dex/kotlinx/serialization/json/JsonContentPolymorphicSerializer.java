...
public abstract class JsonContentPolymorphicSerializer<T> implements KSerializer<T> {
    ...
    public final void serialize(Encoder encoder, T value) {
        ...
        SerializationStrategy actualSerializer = encoder.getSerializersModule().getPolymorphic((KClass<? super KClass<T>>) this.baseClass, (KClass<T>) value);
        if (actualSerializer == null) {
            SerializationStrategy serializerOrNull = SerializersKt.serializerOrNull(Reflection.getOrCreateKotlinClass(value.getClass()));
    ...
    public final T deserialize(Decoder decoder) {
        ...
        DeserializationStrategy<T> selectDeserializer = selectDeserializer(tree);
        ...
        KSerializer actualSerializer = (KSerializer) selectDeserializer;
        return (T) input.getJson().decodeFromJsonElement(actualSerializer, tree);
    }

    private final Void throwSubtypeNotRegistered(KClass<?> kClass, KClass<?> kClass2) {
        String subClassName = kClass.getSimpleName();
        if (subClassName == null) {
            subClassName = String.valueOf(kClass);
        }
        String scope = "in the scope of '" + kClass2.getSimpleName() + '\'';
