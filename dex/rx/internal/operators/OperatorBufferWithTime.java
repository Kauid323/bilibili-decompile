...
public final class OperatorBufferWithTime<T> implements Observable.Operator<List<T>, T> {
    ...
    public Subscriber<? super T> call(Subscriber<? super List<T>> child) {
        ...
        if (this.timespan == this.timeshift) {
            OperatorBufferWithTime<T>.ExactSubscriber parent = new ExactSubscriber(serialized, createWorker);
            parent.add(createWorker);
            child.add(parent);
            parent.scheduleExact();
            return parent;
        }
        OperatorBufferWithTime<T>.InexactSubscriber parent2 = new InexactSubscriber(serialized, createWorker);
        parent2.add(createWorker);
        child.add(parent2);
